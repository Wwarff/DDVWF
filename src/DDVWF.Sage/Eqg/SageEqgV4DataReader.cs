...
public sealed record SageEqgV4LightFx(string Name,string AltName,byte Unknown,float X,float Y,float Z,float RotateX,float RotateY,float RotateZ,float ScaleX,float ScaleY,float ScaleZ,float Unknown1);
public sealed record SageEqgV4Data(IReadOnlyList<SageEqgV4Tile> Tiles,IReadOnlyList<SageEqgV4Placeable> Placeables,IReadOnlyList<SageEqgV4Region> Regions,IReadOnlyList<SageEqgV4TogReference> TogReferences,IReadOnlyList<SageEqgV4LightFx> LightEffects);
...
public static class SageEqgV4DataReader
{
 public static SageEqgV4Data Read(string name,ReadOnlySpan<byte> source,SageEqgV4Header h)
 {
  ...
  return new(tiles,placeables,regions,togs,lightEffects);
 }
