...
public static class SageEqgV4DataReader
{
 public static SageEqgV4Data Read(string name,ReadOnlySpan<byte> source,SageEqgV4Header h)
 {
  ...
  for(var ti=0;ti<tileCount;ti++){
   ...
   var single=checked((int)U());for(var i=0;i<single;i++){var model=S().ToLowerInvariant();_=S();_=U();_=U();var x=F();var y=F();var z=F();var rx=F();var ry=F();var rz=F();var sx=F();var sy=F();var sz=F();_=B();if((unk000&2)!=0)_=U();var th=Height(heights,q,units,x,y);placeables.Add(new(model,y+tileStartY,x+tileStartX,z+th,rx,ry,rz,sx,sy,sz));}
