...
public sealed record SageEqgZone(IReadOnlyList<SageEqgPlaceable> Placeables,IReadOnlyList<SageEqgRegion> Regions,IReadOnlyList<SageEqgLight> Lights);
...
public static class SageEqgZoneReader
{
 public static SageEqgZone Read(string name,ReadOnlySpan<byte> source)
 {
  ...
  var objects=new List<SageEqgPlaceable>(objectCount);const float radToDeg=180f/MathF.PI;
  for(var i=0;i<objectCount;i++){var id=I32();var loc=checked((int)U32());var x=F32();var y=F32();var z=F32();var rx=F32();var ry=F32();var rz=F32();var scale=F32();var modelName=Str(checked(list+loc));var modelFile=id>=0&&id<modelNames.Count?modelNames[id]:"";if(version>1){var n=checked((int)U32());for(var j=0;j<n;j++)_=U32();}objects.Add(new(modelName,modelFile,x,y,z,rx*radToDeg,ry*radToDeg,rz*radToDeg,scale));}
  ...
  return new(objects,regions,lights);
 }
}
