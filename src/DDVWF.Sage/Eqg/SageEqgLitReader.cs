...
public static class SageEqgLitReader
{
 public static IReadOnlyList<uint> Read(string name,ReadOnlySpan<byte> source)
 {
  ...
  var magic=System.Text.Encoding.ASCII.GetString(source[..4]);if(!magic.StartsWith("EQG",StringComparison.Ordinal))throw new InvalidDataException($"EQG lit '{name}' has invalid magic '{magic}'.");
