  ...
  foreach(var f in doc.Fragments.Where(x=>x.KnownType==WldFragmentType.Mesh))
  {
   ...
   if(m.MaterialListIndex>=0&&m.MaterialListIndex<doc.Fragments.Count&&doc.Fragments[m.MaterialListIndex].KnownType==WldFragmentType.MaterialList)
   {
    ...
    foreach(var group in m.MaterialGroups)
    {
     ...
     var pass=polys.Any(x=>!x.IsSolid);var baseName=materials[mi].Name+(pass?"-passthrough":"");var name=baseName;var split=0;while(prims.Any(x=>x.Name.Equals(name,StringComparison.OrdinalIgnoreCase)))name=$"{baseName}-{split++}";var vertices=new List<SageSceneVertex>();var indices=new List<ushort>();var dedup=new Dictionary<(System.Numerics.Vector3,System.Numerics.Vector3,System.Numerics.Vector2,uint),ushort>();
     foreach(var poly in polys)foreach(var idx in new[]{poly.A,poly.C,poly.B}){if(idx>=m.Vertices.Count)throw new InvalidDataException("Mesh polygon references a missing vertex.");var normal=idx<m.Normals.Count?m.Normals[idx]:System.Numerics.Vector3.Zero;var uv=idx<m.Uvs.Count?m.Uvs[idx]:System.Numerics.Vector2.Zero;var color=idx<m.Colors.Count?m.Colors[idx]:0xFFFFFFFF;var v=new SageSceneVertex(SageExportTransform.Position(m.Vertices[idx],m.Center),SageExportTransform.Normal(normal),uv,idx,color);var key=(v.Position,v.Normal,v.Uv,v.Color);if(!dedup.TryGetValue(key,out var di)){if(vertices.Count>=ushort.MaxValue)throw new InvalidDataException("Sage primitive exceeds UInt16 index range.");di=(ushort)vertices.Count;dedup[key]=di;vertices.Add(v);}indices.Add(di);}
