...
public static class WldActorReader
{
    ...
    {
        var p=fragment.PayloadOffset; int I32(){var v=BinaryPrimitives.ReadInt32LittleEndian(file[p..]);p+=4;return v;} uint U32()=>unchecked((uint)I32());
        ...
        var len=checked((int)U32()); if(p+len>file.Length) throw new InvalidDataException("ActorInstance user data exceeds fragment/file bounds.");
        var user=System.Text.Encoding.Latin1.GetString(file.Slice(p,len));
