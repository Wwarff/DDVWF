...
public sealed record SageAmbientLight(uint Flags,IReadOnlyList<int> Regions);
public static class WldLightReader
{
 ...
 public static SageAmbientLight ReadAmbient(WldFragment f,ReadOnlySpan<byte> source){var r=new R(source,f.PayloadOffset);var flags=r.U();var count=checked((int)r.U());var regions=new List<int>(count);for(var i=0;i<count;i++)regions.Add(r.I());return new(flags,regions);}
