...
public sealed record SageAmbientLight(int ReferenceIndex,uint Flags,IReadOnlyList<int> Regions);
public static class WldLightReader
{
 ...
 public static SageAmbientLight ReadAmbient(WldFragment f,ReadOnlySpan<byte> source){var r=new R(source,f.PayloadOffset);var reference=checked((int)r.U()-1);var flags=r.U();var count=checked((int)r.U());var regions=new List<int>(count);for(var i=0;i<count;i++)regions.Add(r.I());return new(reference,flags,regions);}
 ...
 sealed class R{readonly byte[] b;int p;public R(ReadOnlySpan<byte>s,int o){b=s.ToArray();p=o;}void N(int n){if(p+n>b.Length)throw new InvalidDataException("Light fragment is truncated.");}public uint U(){N(4);var v=BinaryPrimitives.ReadUInt32LittleEndian(b.AsSpan(p,4));p+=4;return v;}public int I()=>unchecked((int)U());public float F()=>BitConverter.Int32BitsToSingle(unchecked((int)U()));public byte B(){N(1);return b[p++];}}
}