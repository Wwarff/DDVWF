...
public static class WldMeshReader
{
 public static SageMesh Read(WldDocument doc,WldFragment fragment,ReadOnlySpan<byte> source)
 {
  ...
  if(new[]{vertexCount,uvCount,normalCount,colorCount,polygonCount,vertexPieceCount,groupCount,vertexTextureCount,size9}.Any(x=>x<0))throw new InvalidDataException("Mesh contains a negative collection count.");
  var scale=1f/(1<<exponent);var vertices=new List<Vector3>(vertexCount);
