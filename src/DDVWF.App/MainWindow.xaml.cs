...
public partial class MainWindow:Window
{
 ...
 async Task RenderActiveAsync(){if(_session?.ActiveZone is not{} zone||_client is null)return;var payload=BabylonSceneSerializer.Serialize(zone,_client.RenderMeshes);await ScriptAsync($"window.ddvwf.loadScene({System.Text.Json.JsonSerializer.Serialize(payload)})");}
