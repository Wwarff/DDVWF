...
public sealed record GroundSpawnEntityData(int Version,float MinX,float MaxX,float MinY,float MaxY,float MaxZ,int ItemId,int MaxAllowed,string Comment,int RespawnTimer,bool FixZ,int MinExpansion=-1,int MaxExpansion=-1,string ContentFlags="",string ContentFlagsDisabled="",IReadOnlyList<ObjectContentData>? Contents=null) : ZoneEntityData;
