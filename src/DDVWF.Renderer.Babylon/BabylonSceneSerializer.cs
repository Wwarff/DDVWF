...
public static class BabylonSceneSerializer
{
 public static string Serialize(CompleteZone zone,IReadOnlyDictionary<string,RenderMesh> assets,IReadOnlyDictionary<string,byte[]>? textures=null)
 {
  ...
  var textureData=(textures??new Dictionary<string,byte[]>()).Where(x=>meshes.Any(m=>m.Asset.Primitives.Any(p=>string.Equals(p.TextureName,x.Key,StringComparison.OrdinalIgnoreCase)))).ToDictionary(x=>x.Key,x=>Convert.ToBase64String(x.Value),StringComparer.OrdinalIgnoreCase);
