...
public static class BabylonSceneSerializer
{
 ...
 {
  var meshes=zone.Entities.Where(e=>e.ClientAsset is not null&&assets.ContainsKey(e.ClientAsset)).Select(e=>new{e.Id,e.Name,Kind=e.Kind.ToString(),e.Position,e.Rotation,e.Scale,Asset=assets[e.ClientAsset!]}).ToArray();
  ...
  var markers=zone.Entities.Where(e=>e.Kind is ZoneEntityKind.Door or ZoneEntityKind.ZonePoint or ZoneEntityKind.Npc).Where(e=>e.ClientAsset is null||!assets.ContainsKey(e.ClientAsset)).Select(e=>new{e.Id,e.Name,Kind=e.Kind.ToString(),e.Position,e.Scale,e.ServerId}).ToArray();
  ...
  return JsonSerializer.Serialize(new{zone=zone.ShortName,meshes,markers,lights,textures=textureData},new JsonSerializerOptions{PropertyNamingPolicy=JsonNamingPolicy.CamelCase});
 }
}