...
{
 public static string Serialize(CompleteZone zone,IReadOnlyDictionary<string,RenderMesh> assets,IReadOnlyDictionary<string,byte[]>? textures=null,bool shellOnly=false,bool includeLights=true)
 {
  ...
  var lights=(includeLights?zone.Entities:Enumerable.Empty<ZoneEntity>()).Where(e=>e.Kind==ZoneEntityKind.Light&&e.Data is LightEntityData).Select(e=>new{e.Id,e.Name,e.Position,Data=(LightEntityData)e.Data!}).ToArray();
  var markers=(shellOnly?Enumerable.Empty<ZoneEntity>():zone.Entities).Where(e=>e.Kind is ZoneEntityKind.Door or ZoneEntityKind.ZonePoint or ZoneEntityKind.Npc || e.Data is ObjectEntityData).Where(e=>e.Data is not ObjectEntityData).Where(e=>e.ClientAsset is null||!assets.ContainsKey(e.ClientAsset)).Select(e=>new{e.Id,e.Name,Kind=e.Kind.ToString(),e.Position,e.Scale,e.ServerId,Data=e.Data}).ToArray();
