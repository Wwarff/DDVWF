...
public static class BabylonSceneSerializer
{
 public static string Serialize(CompleteZone zone,IReadOnlyDictionary<string,RenderMesh> assets,IReadOnlyDictionary<string,byte[]>? textures=null)
 {
  ...
  var markers=zone.Entities.Where(e=>e.Kind is ZoneEntityKind.Door or ZoneEntityKind.ZonePoint or ZoneEntityKind.Npc or ZoneEntityKind.GroundSpawn || e.Data is ObjectEntityData).Where(e=>e.ClientAsset is null||!assets.ContainsKey(e.ClientAsset)).Select(e=>new{e.Id,e.Name,Kind=e.Kind.ToString(),e.Position,e.Scale,e.ServerId}).ToArray();
  var groundSpawnAreas=zone.Entities.Where(e=>e.Kind==ZoneEntityKind.GroundSpawn&&e.Data is GroundSpawnEntityData).Select(e=>new{e.Id,e.Name,e.Position,Data=(GroundSpawnEntityData)e.Data!}).ToArray();
